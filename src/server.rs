...
impl TcpServer {
    ...
    pub fn send(&self, send_option: SendOption, data: &[u8]) -> Result<(), String> {
            ...
            match send_option {
                SendOption::All => {
                    clients.iter().for_each(|client| {
                        ...
                        let _ = self.write_to_socket(socket, data);
                    });
                }
                SendOption::AllExcept(id) => {
                    clients.iter().for_each(|client| {
                        if client.id != id {
                            let socket = &client.socket;
                            let _ = self.write_to_socket(socket, data);
                ...
                SendOption::One(id) => {
                    let client = clients.iter().find(|client| client.id == id).unwrap();
                    let socket = &client.socket;
                    let _ = self.write_to_socket(socket, data);
                }
            }
    ...
    fn write_to_socket(&self, socket: &TcpStream, data: &[u8]) -> Result<(), String> {
                        ...
                        if let Ok(mut clients) = data_ref.clients.lock() {
                            if socket.set_nonblocking(true).is_ok() {
                                ...
                                if let Ok(on_client_connected) = data_ref.on_client_connected.lock()
                                {
                                    ...
                                    logger::log_to_file(
                                        "logs.txt",
                                        "OnClientConnectedCallback couldn't be locked!",
                                    );
                                }
                            }
                        }
                ...
                if let Ok(mut clients) = data_ref.clients.lock() {
                    for client in clients.iter_mut() {
                        ...
                        match socket_ref.read(&mut client.buffer[client.read_bytes..]) {
                            Ok(size) => {
                                if size == 0 {
                                    if let Err(e) = data_ref
                                        .sender
                                        .send(ChannelDataType::RemoveClient(client.id))
                                    {
                                        logger::log_to_file(
                                            "logs.txt",
                                            format!("Channel data couldn't be sent!: {e}").as_str(),
                                        );
                                    }
                                    if let Ok(on_client_disconnected) =
                                        data_ref.on_client_disconnected.lock()
                                    {
                                        on_client_disconnected(client.id);
                                    } else {
                                        logger::log_to_file(
                                            "logs.txt",
                                            "OnClientDisconnectedCallback couldn't be locked",
                                        );
                                    }
                                } else {
                                    ...
                                    if amount_to_read > 0
                                        && client.read_bytes == header_size + amount_to_read
                                    {
                                        // consider >= here
                                        if let Ok(on_message_received) =
                                            data_ref.on_message_received.lock()
                                        {
                                            on_message_received(
                                                client.id,
                                                client.buffer[header_size..].to_vec(),
                                            ); // consider header_size..=amount_to_read here
                                            client.buffer.resize(8, 0);
                                            client.read_bytes = 0;
                                        } else {
                                            logger::log_to_file(
                                                "logs.txt",
                                                "OnMessageReceivedCallback couldn't be locked!",
                                            );
                                        }
                                    }
                                }
                                ...
                                Err(e) => {
                                    if e.kind() == io::ErrorKind::UnexpectedEof {
                                            ...
                                            .send(ChannelDataType::RemoveClient(client.id))
                                        {
                                            logger::log_to_file(
                                                "logs.txt",
                                                format!("Channel data couldn't be sent! {e}")
                                                    .as_str(),
                                            );
                                        }
                                        if let Ok(on_client_disconnected) =
